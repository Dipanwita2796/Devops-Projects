...
authentication=$(curl -i POST "https://$host_address/tenant-management-api//auth/signin" -H "accept: */*" -H "Content-Type: application/json" -d "{ \"pwd\": \"$pwd\", \"uid\": \"$username\"}" )
modauthentication=$(echo $authentication |sed -e 's/:/ /g' -e 's/,/ /g' -e 's/"/ /g')
token=$(echo $modauthentication |grep -oP "(?<=body )[^ ]+")
modtoken="Bearer "$token
#echo $modtoken

echo "Fetching metadata from csv for the documents">>log_file/log_$DATETIME.txt

cd .. && . ./upload_with_diff_metadata_new.sh
...
do
    ...
    result="$(curl -k -i -H "Content-Type=multipart/form-data" -F "files=@"$eachfile";type=application/pdf" -X POST https://$host_address/dmsnew-api/dms/fileupload -H "Authorization: $modtoken")"
	response=(${result[@]})
	...
	finindex_code=$(echo $modresponse |grep -oP "(?<=finendmonthindex_code )[^ ]+")
	finindex_code_mod=$(echo $finindex_code |sed -e 's/-//g')
	
	finindex_name=$(echo $modresponse |grep -oP "(?<=finendmonthindex_name )[^ ]+")
	finindex_name_mod=$(echo $finindex_name |sed -e 's/-//g')
	
	country_code=$(echo $modresponse |grep -oP "(?<=country_code )[^ ]+")
	country_code_mod=$(echo $country_code |sed -e 's/-//g')
	
	country_name=$(echo $modresponse |grep -oP "(?<=country_name )[^ ]+")
	country_name_mod=$(echo $country_name |sed -e 's/-//g')
	...
	sed -i 's/fin_value/'$finindex_name_mod'/g' customer_copy.json
	sed -i 's/fin_code/'$finindex_code_mod'/g' customer_copy.json
	sed -i 's/country_code/'$country_code_mod'/g' customer_copy.json
	sed -i 's/country_value/'$country_name_mod'/g' customer_copy.json
	...
	curl -X POST "https://$host_address/doc-gateway-api/customers" -H "accept: */*" -H "Authorization: $modtoken" -H "Content-Type: application/json" -d @customer_copy.json>>log_file/log_$DATETIME.txt
	
	customer_search="$(curl -X GET "https://$host_address/doc-gateway-api//customers/search/$entity_idmod?isExactMatch=true" -H "accept: */*" -H "Authorization: $modtoken")"
	customer_search_mod=$(echo $customer_search |sed -e 's/:/ /g' -e 's/,/ /g' -e 's/{/ /g' -e 's/}/ /g' -e 's/"/ /g')
	internalentityid=$(echo $customer_search_mod |grep -oP "(?<=internalentityid )[^ ]+")
	#echo $internalentityid
	...
	then
		...
		sed -i 's/features_value/'$featuresmod'/g' upload_copy.json
	else
	...
	sed -i 's/dms_code/'$documentId'/g' upload_copy.json
		...
		sed -i 's/entity_id/'$internalentityid'/g' upload_copy.json
		sed -i 's/entity_name/'$entities'/g' upload_copy.json

		if [ $flag == 0 ]
		then
	      echo "Proccessing Initiated for " $eachfile>>log_file/log_$DATETIME.txt
		  curl -X POST -H "Content-Type: application/json" -d @upload_copy.json https://$host_address/doc-gateway-api/documents -H "Authorization: $modtoken">>log_file/log_$DATETIME.txt
	    fi
	rm $eachfile
